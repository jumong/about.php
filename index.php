...
function game($a, $b){ //Функция: Алгоритм игры
     if ( count($_GET) > 0 ){ // Проверяем получено ли значение
	     if ($b > $a){ // Условие проверяющее введенное значение пользователя, больше задуманного числа или нет
		     return '<p>Много</p><p>Попробуй еще разочек</p>'; // если да то возвращаем сообщение "Много" и Просим повторить попытку
	     }
		     ...
		     if($b < $a){ // Проверяем значение пользователя, меньше задуманного числа или нет
			     return '<p>Мало</p><p>Попробуй еще разочек</p>'; //если да то возвращаем сообщение "Мало"  и Просим повторить попытку
		     }
