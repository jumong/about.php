...
function game($a, $b){ //Функция: Алгоритм игры
     if ( count($_GET) > false ){ // Проверяем получено ли значение
		...
		if ($b > $a){ // Условие проверяющее введенное значение пользователя больше ли оно задуманного числа или нет     
		   return '<p>Упс слишком много</p><p>Попробуй еще разочек, но будь внимательнее с каждой ошибкой задуманное число меняется</p>'; // если да то возвращаем сообщение "Много" и Просим повторить попытку
		}
	        elseif($b < $a){ // Проверяем значение пользователя, меньше ли оно задуманного числа или нет
		   return '<p>Упс слишком мало</p><p>Попробуй еще разочек, но будь внимательнее с каждой ошибкой задуманное число меняется</p>'; //если да то возвращаем сообщение "Мало"  и Просим повторить попытку
		}
